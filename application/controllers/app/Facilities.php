...
class Facilities extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load_view	= $this->config->item('backend_path').'facilities/';
	...
	{
		$data = array();
		$data['field'] 		= array('NO.', 'Facilities', 'Icon', 'Description', 'Actions');

		$data['page_title']  = 'Facilities';

		$data['parent_menu'] = 'master';
	...
	public function detail()
	{
		...
		$facilities = $this->modeldb->get_list($this->table, ['status' => 1, 'flag' => 1, 'id' => $id]);

		echo json_encode($facilities[0]);
	...
	public function delete($id)
	{
		...
		redirect(backend_url('facilities'));
	}
	...
	public function get_data()
	{
		...
		{	
			...
			$raw[] 	= $val->icon;
			$raw[] 	= $val->description;
			$raw[]	= '<a href="'.backend_url('facilities/delete').'/'.$val->id.'" onclick="return confirmDelete($(this));" class="btn btn-simple btn-danger btn-icon remove pull-right"><i class="material-icons">close</i></a>
