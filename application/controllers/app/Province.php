...
class Province extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load_view	= $this->config->item('backend_path').'province/';
	...
	{
		$data = array();
		$data['country'] 	 = $this->modeldb->get_list('country', ['status' => 1, 'flag' => 1]);
		$data['field'] 		 = array('NO.', 'Province', 'Description', 'Actions');
	...
	public function detail()
	{
		...
		$province 	= $this->modeldb->get_list($this->table, ['status' => 1, 'flag' => 1, 'id' => $id]);
		echo json_encode($province[0]);
	}
	...
	public function get_data()
	{
		...
		{	
			...
			$raw[] 	= $val->name;
			$raw[] 	= $val->description;
			$raw[]	= '<a href="'.backend_url('province/delete').'/'.$val->id.'" onclick="return confirmDelete($(this));" class="btn btn-simple btn-danger btn-icon remove pull-right"><i class="material-icons">close</i></a>
