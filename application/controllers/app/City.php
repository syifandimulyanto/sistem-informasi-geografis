...
class City extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load_view	= $this->config->item('backend_path').'city/';
	...
	{
		$data = array();
		$data['province'] 	 = $this->modeldb->get_list('province', ['status' => 1, 'flag' => 1]);
		$data['field'] 		 = array('NO.', 'City', 'Description', 'Actions');
	...
	public function detail()
	{
		...
		$city 		= $this->modeldb->get_list($this->table, ['status' => 1, 'flag' => 1, 'id' => $id]);
	...
	public function get_data()
	{
		...
		{	
			...
			$raw[] 	= $val->name;
			$raw[] 	= $val->description;
			$raw[]	= '<a href="'.backend_url('city/delete').'/'.$val->id.'" onclick="return confirmDelete($(this));" class="btn btn-simple btn-danger btn-icon remove pull-right"><i class="material-icons">close</i></a>
