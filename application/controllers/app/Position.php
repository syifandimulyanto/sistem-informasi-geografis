...
class Position extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load_view	= $this->config->item('backend_path').'position/';
	...
	{
		$data = array();
		$data['position'] 	= $this->modeldb->get_list($this->table, ['status' => 1, 'flag' => 1]);
		$data['field'] 		= array('NO.', 'Position', 'Description', 'Actions');

		$data['page_title']  = 'Position';

		$data['parent_menu'] = 'position';
		$data['menu']		 = 'position';
	...
	public function detail()
	{
		...
		$position 	= $this->modeldb->get_list($this->table, ['status' => 1, 'flag' => 1, 'id' => $id]);
	...
	public function get_data()
	{
		...
		{	
			...
			$raw[] 	= $val['name'];
			$raw[] 	= $val['description'];
			$raw[]	= '<a href="'.backend_url('position/delete').'/'.$val['id'].'" onclick="return confirmDelete($(this));" class="btn btn-simple btn-danger btn-icon remove pull-right"><i class="material-icons">close</i></a>
	...
	public function position_tree($parent = 0, $spacing = '', $position_array = '') 
    {
        if (!is_array($position_array))
            $position_array = array();
     	
     	$position = $this->modeldb->get_list($this->table, ['status' => 1, 'flag' => 1, 'parent_id' => $parent]);
        if(count($position) > 0)
        {
            foreach ($position as $key => $val) 
            {
                $position_array[]  = array(
                                                'id'            => $val->id, 
                                                'name'          => $spacing . ' '.$val->name, 
                                                'description'   => $val->description,
                                                'parent_id'     => $val->parent_id,
                                            );
                $position_array = $this->position_tree($val->id, '&nbsp;&nbsp;&nbsp;&nbsp;'.$spacing . '--', $position_array);
            }
        } 
